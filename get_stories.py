...
aylien_api_key = st.text_input(
    'Aylien API key')
aylien_application_id = st.text_input('Aylien Application ID', '')
included_source_domains = st.multiselect(
    'Sources', source_domains, default_source_domains)
included_entities = st.multiselect(
    'Included entities', entities, default_included_entity_types)
excluded_entities = st.multiselect(
    'Excluded entities', entities, default_excluded_entity_types)
sentiment_polarity = st.radio(
...
if go:
    try:
        ...
        for domain in included_source_domains:
            api_response = get_stories(
                domain, included_entities, excluded_entities, sentiment_polarity, per_page)
            for story in api_response.stories:
                stories += [[story.source.domain, story.title]]
        stories = pandas.DataFrame(data=stories, columns=[
                                   'Source', 'Headline'])
        st.subheader('Stories')
        st.dataframe(stories)

        csv = stories.to_csv(index=False)
        b64 = base64.b64encode(csv.encode()).decode()
        href = f'<a download="headlines.csv" href="data:file/csv;base64,{b64}">Download CSV</a>'
